...
const Sidebar = () => {
  ...
  return (
    <div className={`relative h-screen transition-all duration-300 ease-in-out ${isOpen ? 'w-72' : 'w-20'}`}>
      {/* Sidebar Container */}
      <div className={`h-full bg-[#0f0f0f] text-[#f5f5f5] flex flex-col border-r border-[#2a2a2a]`}>
        
        {/* Header with new chat button */}
        <div className="p-5 flex items-center justify-between border-b border-[#2a2a2a]">
          <div 
            className={`flex items-center space-x-3 overflow-hidden transition-all ${isOpen ? 'w-full' : 'w-0 opacity-0'}`}
            onMouseEnter={() => setIsHoveringLogo(true)}
            onMouseLeave={() => setIsHoveringLogo(false)}
          >
            <div className={`bg-gradient-to-r from-[#d4af37] to-[#f1c232] p-2 rounded-lg transition-transform duration-300 ${isHoveringLogo ? 'rotate-6 scale-105' : ''}`}>
              {/* Your logo SVG */}
            </div>
          ...
          <button 
            ...
            className="p-2 rounded-lg hover:bg-[#2a2a2a] transition-all duration-200 text-[#d4af37] hover:text-[#f1c232] hover:rotate-90"
          >
        ...
        <button
          ...
          disabled={loading}
          className={`mx-3 my-4 p-2 rounded-lg bg-gradient-to-r from-[#d4af37] to-[#f1c232] text-[#0f0f0f] font-medium flex items-center justify-center transition-all duration-300 ${
            loading ? 'opacity-50 cursor-not-allowed' : 'hover:from-[#f1c232] hover:to-[#d4af37]'
          }`}
        >
              ...
              {chats.map((chat) => (
                <li key={chat._id}>
                  ...
                  >
                    ...
                    <span className={`ml-3 whitespace-nowrap text-left transition-all ${
        ...
        <div className="mt-auto p-4 border-t border-[#2a2a2a] bg-gradient-to-t from-[#1a1a1a]/80 to-transparent">
          <button
            ...
            className={`w-full flex items-center p-3 rounded-lg hover:bg-[#2a2a2a]/50 transition-all duration-200 ${
